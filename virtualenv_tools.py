...
def update_activation_script(script_filename, new_path):
    ...
    if changed:
        print('A {0!s}'.format(script_filename))
...
def update_script(script_filename, new_path):
    ...
    args[0] = new_bin
    lines[0] = '#!%s\n' % ' '.join(args)
    print('S {0!s}'.format(script_filename))
    with open(script_filename, 'w') as f:
        f.writelines(lines)
...
def update_paths(base, new_path):
    ...
    if not os.path.isabs(new_path):
        print('error: {0!s} is not an absolute path'.format(new_path))
        return False
    ...
    if lib_dir is None or not os.path.isdir(bin_dir) \
       or not os.path.isfile(os.path.join(bin_dir, 'python')):
        print('error: {0!s} does not refer to a python installation'.format(base))
        return False
...
def reinitialize_virtualenv(path):
    ...
    if not os.path.isdir(lib_dir):
        print('error: {0!s} is not a virtualenv bin folder'.format(path))
        return False
    ...
    if py_ver is None:
        print('error: could not detect python version of virtualenv {0!s}'.format(path))
        return False
    ...
    if not sys_py_executable:
        print('error: could not find system version for expected python version {0!s}'.format(py_ver))
        return False
